...
###   iridiumcore.org and iridium.org and compares them.
...
WORKINGDIR="/tmp/iridium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://iridiumcore.org"
HOST2="https://iridium.org"
BASEDIR="/bin/"
VERSIONPREFIX="iridium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'iridium-',
...
if [ $? -ne 0 ]; then
   echo "iridium.org failed to provide signature file, but iridiumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "iridium.org and iridiumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Iridium Core binary release signing key installed?"
   fi
