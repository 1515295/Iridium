...
The RPM spec file provided here is for Iridium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://iridium.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Iridium git repository](https://github.com/iridium/iridium) but are not part of
...
Currently only `Source10` is used, for the example `iridium.conf` file.
...
in the iridium git repository. At present only an SVG version of the Iridium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the iridium
...
needed when 0.12.1 is released, a proper fix is already in the Iridium git
master branch.
...
idea to build Iridium Core against a static version of that library compiled
...
The `iridium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `iridium-server` package. At this point, most RPM based
...
The `iridium-server` package should have SELinux support. How to properly do
...
Iridium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/iridium/). He has been
packaging Iridium for Fedora at least since 2012.
...
4. I renamed the `iridium` package that contains the Qt GUI to `iridium-core` as
that appears to be how the general population refers to it, in contrast to
`iridium-xt` or `iridium-classic`. I wanted to make sure the general population
